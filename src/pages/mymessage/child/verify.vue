<template>
  <div>
	...
	<div class="wrapper7" ref="wrapper7">
  <div class="content">
...
export default{
  ...
  methods:{
      initNewData(){
        let   self=this;
          this.$ajax.post("get/newvbynumber",{location: self.location,number: self.number},function(userdata){   
  ...
  bottom: false
          ...
          let   self=this;
           self.scroll.on('touchEnd', (pos) => {
                ...
                if (pos.y > 30) {
                  ...
                  console.log("sss");
               //刷新
                  self.$ajax.post("get/newvbynumber",{location:0,number:self.number},function(userdata){ 
   ...
   self.$ajax.post("get/newvbynumber",{location: self.location,number: self.number},function(userdata){   
       ...
       this.pushto('/newv',{id:newid,user:userid})
  ...
  mounted(){
      ...
      if(parseInt(localStorage.getItem('permission'))<3)
        {alert("您没有权限");this.back();}
